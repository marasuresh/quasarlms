...
<asp:Repeater runat="server" ID="rptRequests"
    ...
    <HeaderTemplate>
        <table cellspacing="0" cellpadding="3" align="center">
            <tr>
                <th>
                    <%= Resources.CourseRequests.tableExisdataTRequests_tableHeader_03 %>
                </th>
                <th>
                </th>
            </tr>
    ...
    <ItemTemplate>
        <tr bgcolor='<%# Container.ItemIndex % 2  == 1 ? "#FFFFFF" : "#F6F6F6" %>'>
            <td title="<%# Eval("Course.DescriptionUrl") %>">
                ...
                <em>
                    <%# Eval("Comments") %></em> <small>
                        <%# ((Request)Container.DataItem).Published.Value.ToShortDateString() %></small>
            </td>
            <td>
                <asp:ImageButton runat="server" ID="lbDeleteRequest" ImageUrl="~/Lms/UI/Img/03/01.png"
                    CommandArgument='<%# Eval("ID") %>' CommandName="DeleteRequest" AlternateText="<%$ Resources: CourseRequests, tableExisdataTRequests_tableHeader_01 %>" />
            </td>
        </tr>
    </ItemTemplate>
</asp:Repeater>
