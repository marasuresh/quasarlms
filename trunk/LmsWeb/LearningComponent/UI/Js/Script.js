	...
	function DeletePackages() {
		...
		// will be called
		var args = {
			PackagesToDelete: ForEachSelectionCheckbox(false, false)
		};
	...
	function ForEachSelectionCheckbox(fSelect, fDeselect) {
		...
		var aSelected = $(':checkbox[id^="Select"][id!="SelectAll"]')
			.map(function(i, input) {
				input.checked |= fSelect;
				input.checked ^= fDeselect;
	...
	function DeletePackages_UpdateParentPageScript() {
	...
	function DeletePackages_CloseDialogScript() {
	...
	function DeletePackages_Main() {
		...
		$.extend(dialogArguments, opener.dialogArguments);
	}
