...
//	m_pCompare = GetCZipStrCompFunc(bCaseSensitive);
	m_pCompare = & CZipString::Compare2NoCase;
...
{
   const TCHAR * lpszFileName = pHeader->GetFileNameStr();
	//const CZipString & fileName = pHeader->GetFileNameStr();
	int iSize = m_findarray.GetSize();
...
//		int result = CompareElement(fileName, (WORD)midpoint);
      const TCHAR * str = m_findarray[midpoint].m_pHeader->GetFileNameStr();
      int result = _tcsicmp(str,lpszFileName);
