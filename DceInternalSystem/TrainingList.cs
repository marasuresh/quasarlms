...
namespace DCEInternalSystem
{
   ...
   /// </summary>
   public class TrainingControl : NodeControl
   {
      public TrainingControl   (NodeControl parent)
            : base(parent)
      {
      ...
      public override System.Windows.Forms.UserControl GetControl()
      {
         ...
         {
            this.fControl = new DCEInternalSystem.TrainingList(this);
         }
         if (needRefresh)
         {
            ((TrainingList)this.fControl).RefreshData();
            needRefresh = false;
         }
   ...
   /// <summary>
	/// Список тренингов
	/// </summary>
	public class TrainingList : System.Windows.Forms.UserControl
	{
      ...
      private System.Windows.Forms.MenuItem NewTraining;
      ...
      private System.Windows.Forms.MenuItem menuItemExpire;

      private TrainingControl Node;
		public TrainingList(TrainingControl node)
		{
...
where t.Expires=0 and t.Course = c.id","Trainings");
		...
		private void InitializeComponent()
		{
         ...
         this.NewTraining = new System.Windows.Forms.MenuItem();
         ...
         this.menuItemExpire = new System.Windows.Forms.MenuItem();
         ...
         // NewTraining
         // 
         this.NewTraining.Index = 2;
         this.NewTraining.Text = "Новый тренинг";
         this.NewTraining.Click += new System.EventHandler(this.menuItem1_Click);
         // 
         // RemoveTraining
         // 
         this.RemoveTraining.Index = 3;
         ...
         this.toolBar1.Size = new System.Drawing.Size(572, 40);
         ...
         this.dataList.Size = new System.Drawing.Size(572, 280);
         ...
         this.dataColumnHeader2.Width = 304;
         // 
         // dataColumnHeader5
         // 
         ...
         // menuItemExpire
         // 
         this.menuItemExpire.Index = 4;
         this.menuItemExpire.Text = "Переместить в завершенные";
         this.menuItemExpire.Click += new System.EventHandler(this.menuItemExpire_Click);
         // 
         // TrainingList
         // 
      ...
      {
         TrainingEditControl t = new TrainingEditControl(this.Node,"");
         t.Select();
      }
      ...
      public void RefreshData()
      {
         if (DCEUser.CurrentUser.Trainings == DCEUser.Access.No)
            dataSet = DCEAccessLib.DCEWebAccess.WebAccess.GetDataSet(
               @"select dbo.GetStrContentAlt(t.Name, 'RU', 'EN') as RName, 
dbo.GetStrContentAlt(c.Name, 'RU', 'EN') as RCourse , t.* from Trainings t, Courses c
where t.Course = c.id and t.id in (SELECT tr.id from Trainings tr, GroupMembers gm where tr.Expires=0 and gm.id='"+DCEUser.CurrentUser.id+"' and (gm.MGroup = tr.Instructors or gm.MGroup = tr.Curators))","Trainings");
         else
         dataSet = DCEAccessLib.DCEWebAccess.WebAccess.GetDataSet(
            @"select dbo.GetStrContentAlt(t.Name, 'RU', 'EN') as RName, 
dbo.GetStrContentAlt(c.Name, 'RU', 'EN') as RCourse , t.* from Trainings t, Courses c
where t.Expires=0 and t.Course = c.id","Trainings");
         ...
         if (this.dataList.SelectedItems.Count>0 && this.dataList.SelectedItems[0].Tag!=null)      
         {
            ...
            foreach( NodeControl node in this.Node.Nodes)
            {
               ...
               if (node.GetType() == typeof(TrainingScheduleControl) )
                  ...
                  {
                     System.Windows.Forms.MessageBox.Show("Перед удалением тренинга, закройте окно редактирования.","Удалить");
                     node.Select();
                     candelete = false;
                     break;
                  }
      ...
      private void menuItemExpire_Click(object sender, System.EventArgs e)
      {
         if (this.dataList.SelectedItems.Count>0 && this.dataList.SelectedItems[0].Tag!=null)      
         {
            DataRowView row = (DataRowView)this.dataList.SelectedItems[0].Tag;
            ...
            foreach( NodeControl node in this.Node.Nodes)
            {
               if (node.GetType() == typeof(TrainingEditControl) )
               {
                  if ( ((TrainingEditControl)node).Id == row["id"].ToString())
                  {
                     System.Windows.Forms.MessageBox.Show("Перед перемещением редактируемого тренинга, закройте окно редактирования.","Переместить");
                     node.Select();
                     candelete = false;
                     break;
                  }
            ...
            if (candelete)
            {
               DCEAccessLib.DCEWebAccess.WebAccess.ExecSQL("update Trainings set Expires=1 where id='"+row["id"].ToString()+"'");
               RefreshData();
            }
         }
